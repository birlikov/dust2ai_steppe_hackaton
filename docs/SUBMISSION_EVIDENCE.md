...
## Channel coverage — runtime persona, end-to-end (2026-05-09)

All three customer channels (WhatsApp, Instagram DM, Google Business
reviews) were exercised by `scripts/test_persona_channels.py` —
orchestrator → `ClaudeBridge` → `claude -p` (Opus 4.7) → MCP tool calls
— against the **live** Steppe Business Club MCP. The replies below are
generated by the runtime persona on each call, not canned strings. Raw
artefact: `data/scorecard_persona_20260510T024005Z.json`.

**WhatsApp** (proof: replied with real availability + price after a
`square_list_catalog` call; no fabricated stock numbers):

> Good news — both are on the counter today:
>
> - cake "Honey", slice — $8.50, 72 in stock
> - cake "Honey", whole — $55, 10 in stock
>
> Either one is ready for pickup now (about 45 minutes if you'd like a
> fresh whole cake set aside). Want me to hold one for you?

**Instagram DM** (proof: refused to fabricate Napoleon availability —
`agent/RULES.md` r2 "no fabricated data" — and offered a real fallback
plus owner escalation):

> Hi — honestly, the cake "Napoleon" isn't on our counter today. For 6
> people, our closest fit is the whole honey cake at $55, or six
> honey-cake slices at $8.50 each ($51 total). Want me to check with
> Saule on when the cake "Napoleon" comes back?

**Google Business review reply** (proof: brand-voice closing pattern
from brandbook §3, names the owner, no banned adjectives):

> Thank you, friend — that means a lot. Saule will be glad to hear it.
> The cake "Honey" will be on the counter when you're ready for the next
> one.
>
> — the HappyCake team
...
| Metric | Value |
|---|---|
| `evaluator_score_channel_response` | **80 / 100** |
| `evaluator_score_world_scenario` | **100 / 100** |
| `whatsappInbound` | 8 |
| `auditCalls` | 200 |
