...
| `square_create_order` | After the customer confirms what + how-many + channel. **Always followed by `kitchen_create_ticket`.** | Required: `items[{variationId, quantity}]`. Optional: `source` (one of `website | whatsapp | instagram | walk-in | agent`), `customerName`, `customerNote`. **Always include `idempotencyKey: f"{customer_external_id}:{sorted_slugs_joined}:{date_yyyy_mm_dd}"`** per RULES.md rule 12 — same customer asking for the same cake on the same day must not be billed twice (Sean's whole Honey was placed twice in one night; don't repeat it). Returns an `orderId` you must pass to the kitchen. |
